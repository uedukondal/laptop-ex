...
# User inputs (corrected column names)
...
storage = st.slider("Total Storage (GB)", 128, 2048, 256)
ssd = st.radio("Has SSD?", [0, 1])
hdd = st.radio("Has HDD?", [0, 1])
graphic_vram = st.slider("GPU VRAM (GB)", 0, 12, 0)
battery = st.slider("Battery Capacity (WHr)", 30, 100, 70)
